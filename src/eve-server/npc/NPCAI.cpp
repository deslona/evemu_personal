            ...
            if( m_beginFindTarget.Check() )
            {
                ...
                for(; cur != end; cur++)
                {
                    ...
                    if( (*cur)->IsClient() )
                    {
                        ...
                        if( (currentClient->Destiny()) != NULL )
                        {
                            //if( !((*cur)->CastToClient()->Destiny()->IsCloaked()) )
                            if( !(currentClient->Destiny()->IsCloaked()) )
                            {
...
void NPCAIMgr::CheckAttacks(SystemEntity *target) {
    if(m_mainAttackTimer.Check(false)) {
        ...
        if( target->IsClient() )
        {
            Client * const currentTarget = target->CastToClient();
            if( currentTarget->Destiny() != NULL )
            {
                if( currentTarget->Destiny()->IsCloaked() )
                {
