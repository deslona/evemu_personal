...
PyResult Command_spawn( Client* who, CommandDB* db, PyServiceMgr* services, const Seperator& args )
{
    ...
    for(spawnIndex=0; spawnIndex < spawnCount; spawnIndex++)
    {
        ...
        if( groupID == 84 || groupID == 88 || groupID == 89 || groupID == 90 || groupID == 384 || groupID == 385 || groupID == 386 || groupID == 387 || groupID == 395 || groupID == 396 || groupID == 476 || groupID == 648 || groupID == 653 || groupID == 654 || groupID == 655 || groupID == 656 || groupID == 657 || groupID == 772 || groupID == 1019)
       {
           // This is NOT even going to make it here since Missiles/Torpedos/Bombs/etc are not supported yet by SystemManager::BuildDynamicEntity()
               sLog.Success( "Command_spawn", "Missile Type Spawned" );
       }
    }
...
PyResult Command_giveallskills( Client* who, CommandDB* db, PyServiceMgr* services, const Seperator& args ) {
    ...
    if( args.argCount() >= 2 ) {
        if( args.isNumber( 1 ) ) {
            ...
            client = services->entity_list.FindCharacter( ownerID );
            if( client == NULL )
