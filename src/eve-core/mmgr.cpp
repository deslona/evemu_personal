...
static	sAllocUnit	*findAllocUnit(const void *reportedAddress)
{
	...
	unsigned long	hashIndex = (reinterpret_cast<unsigned long>(const_cast<void *>(reportedAddress)) >> 4) & (hashSize - 1);
	...
	for (unsigned int i = 0; i < hashSize; i++)
	{
		...
		while(ptr)
		{
			fprintf(fp, "%06d 0x%08X 0x%08X 0x%08X 0x%08X 0x%08X %-8s    %c       %c    %s\r\n",
				...
				reinterpret_cast<unsigned long>(ptr->reportedAddress), ptr->reportedSize,
				reinterpret_cast<unsigned long>(ptr->actualAddress), ptr->actualSize,
				m_calcUnused(ptr),
...
void	*m_allocator(const char *sourceFile, const unsigned int sourceLine, const char *sourceFunc, const unsigned int allocationType, const size_t reportedSize)
{
	try
	{
		...
		unsigned int	hashIndex = (reinterpret_cast<unsigned long>(au->reportedAddress) >> 4) & (hashSize - 1);
		...
		if (alwaysLogAll) log("[+] ---->             addr 0x%08X", reinterpret_cast<unsigned long>(au->reportedAddress));
...
void	*m_reallocator(const char *sourceFile, const unsigned int sourceLine, const char *sourceFunc, const unsigned int reallocationType, const size_t reportedSize, void *reportedAddress)
{
	try
	{
		...
		if (oldReportedAddress != au->reportedAddress)
		{
			...
			{
				unsigned int	hashIndex = (reinterpret_cast<unsigned long>(oldReportedAddress) >> 4) & (hashSize - 1);
			...
			hashIndex = (reinterpret_cast<unsigned long>(au->reportedAddress) >> 4) & (hashSize - 1);
			if (hashTable[hashIndex]) hashTable[hashIndex]->prev = au;
			au->next = hashTable[hashIndex];
			au->prev = NULL;
			hashTable[hashIndex] = au;
...
void	m_deallocator(const char *sourceFile, const unsigned int sourceLine, const char *sourceFunc, const unsigned int deallocationType, const void *reportedAddress)
{
	try
	{
		...
		if (alwaysLogAll) log("[-] ----- %8s of addr 0x%08X           by %s", allocationTypes[deallocationType], reinterpret_cast<unsigned long>(const_cast<void *>(reportedAddress)), ownerString(sourceFile, sourceLine, sourceFunc));
